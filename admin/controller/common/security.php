...
class Security extends \Opencart\System\Engine\Controller {
	...
	public function getList(): string {
		...
		$admin_path = rtrim(str_replace('\\', '/', $path), '/');
		$application_path = rtrim(str_replace('\\', '/', DIR_APPLICATION), '/');
		$is_default_admin = DIRECTORY_SEPARATOR === '\\' ? strcasecmp($application_path, $admin_path) === 0 : $application_path === $admin_path;
		...
		if (is_dir($path) && !$is_default_admin) {
	...
	public function admin(): void {
		...
		if (!$json) {
			...
			$blocked = ['admin', 'catalog', 'extension', 'image', 'install', 'system'];
		...
		if (!$json) {
			...
			foreach (array_slice($files, $start, $end - $start) as $file) {
				if ($json) {
					break;
				}
