<?php
namespace Opencart\App\Controller\Tool;
/**
 * Class RuntimeDiagnostics
 *
 * @package Opencart\App\Controller\Tool
 */
class RuntimeDiagnostics extends \Opencart\System\Engine\Controller {
	...
	private function resolveTrigger(string $trigger): string {
		...
		if (!preg_match('/^(app|api)\/(model|controller)\/([a-z0-9_]+(?:\/[a-z0-9_]+)*)\.([a-zA-Z0-9_]+)\/(before|after)$/', $trigger, $match)) {
			return 'invalid';
		}

		$directory = $match[1] == 'app' ? DIR_APPLICATION : DIR_API;
	...
	private function resolveAction(string $trigger, string $action): string {
		if (str_starts_with($trigger, 'app/')) {
			$directory = DIR_APPLICATION;
		} elseif (str_starts_with($trigger, 'api/')) {
			$directory = DIR_API;
		} else {
			return 'invalid';
		}
