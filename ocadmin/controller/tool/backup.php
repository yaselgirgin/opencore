...
class Backup extends \Opencart\System\Engine\Controller {
	...
	public function backup(): void {
		...
		if (!is_array($backup) || !$backup || array_diff($backup, $allowed)) {
		...
		if (!$json) {
			try {
				...
				$this->log->write('Manual complete database backup failed: ' . $throwable->getMessage());
		...
		if (!$json) {
			...
			if (is_dir($file)) {
				try {
					...
					$this->log->write('Manual complete database restore failed: ' . $throwable->getMessage());
					$json['error'] = $this->language->get('error_file');
				}
